...
namespace DAP.DWSupport.Endpoints
{
    ...
    using MyRepository = Repositories.DrydockSuppRepository;
    ...
    [RoutePrefix("Services/DWSupport/DrydockSupp"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class DrydockSuppController : ServiceEndpoint
    {
        ...
        public FileContentResult ExcelExport(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.DrydockSuppColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "DryDock" +
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            if (!importedHeaders.Contains(MyRow.Fields.ShipCd.Title, StringComparer.OrdinalIgnoreCase) == true)
            {
                response.ErrorList.Add("Missing Required Column: " + MyRow.Fields.ShipCd.Title);
            }
            if (!importedHeaders.Contains(MyRow.Fields.StartDate.Title, StringComparer.OrdinalIgnoreCase) == true)
            {
                response.ErrorList.Add("Missing Required Column: "+ MyRow.Fields.StartDate.Title);
            }
            if (!importedHeaders.Contains(MyRow.Fields.EndDate.Title, StringComparer.OrdinalIgnoreCase) == true)
            {
                response.ErrorList.Add("Missing Required Column: " + MyRow.Fields.EndDate.Title);
            }
            if (!importedHeaders.Contains(MyRow.Fields.NumberOfDays.Title, StringComparer.OrdinalIgnoreCase) == true)
            {
                response.ErrorList.Add("Missing Required Column: " + MyRow.Fields.NumberOfDays.Title);
            }
            ...
            * the column location and match it to the system header using a Dictionary<string, int>. */
            ...
            for (var row = 2; row <= wsEnd.Row; row++)
            {
                try
                {
                    ...
                    // Load record as string and parse manually later to date
                    entType = jImpHelp.entryType.String;            //excel field type
                    fieldTitle = myFields.StartDate.Title.ToLower();           //excel field name
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            start_date = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    // Load record as string and parse manually later to date
                    entType = jImpHelp.entryType.String;            //excel field type
                    fieldTitle = myFields.EndDate.Title.ToLower();           //excel field name
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            end_date = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    var dt1 = DateTime.FromOADate(Double.Parse(start_date));
                    var dt2 = DateTime.FromOADate(Double.Parse(end_date));
                    var currentRow = uow.Connection.TryFirst<DrydockSuppRow>(q => q
                        .Select(myFields.DrydockId)
                        .Where(
                            myFields.ShipCd == ship_cd
                            &&
                            myFields.StartDate == dt1
                            &&
                            myFields.EndDate == dt2
                        )
                    );

                    // Row doesn't exist in DB
                    if (currentRow == null)
                        currentRow = new DrydockSuppRow
                        {
                            ShipCd = ship_cd,
                            StartDate = dt1,
                            EndDate = dt2
                    ...
                    fieldTitle = myFields.NumberOfDays.Title.ToLower();  //excel field name
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.NumberOfDays = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    entType = jImpHelp.entryType.Int;            //excel field type
                    fieldTitle = myFields.SailYear.Title.ToLower();         //excel field name
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.SailYear = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    entType = jImpHelp.entryType.String;            //excel field type
                    fieldTitle = myFields.Status.Title.ToLower();         //excel field name
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.Status = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    if (currentRow.DrydockId == null)
                    {
                        new DrydockSuppRepository().Create(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow
                        });
                    ...
                    {
                        new DrydockSuppRepository().Update(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow,
                            EntityId = currentRow.DrydockId.Value
                        });
