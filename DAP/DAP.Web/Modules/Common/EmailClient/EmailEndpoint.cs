...
namespace Serenity.EmailClient.Endpoints
{
    ...
    [ServiceAuthorize]
    [RoutePrefix("Services/Common/Email"), Route("{action=index}")]
    ...
    public class EmailController : ServiceEndpoint
    {
        ...
        public ServiceResponse Login(EmailLoginRequest request)
        {
            ...
            try
            {
                ...
                var token = Convert.ToBase64String(MachineKey.Protect(bytes, "EmailClientAuth"));
                Response.Cookies.Add(new HttpCookie("EmailClientLogin_" + Authorization.UserId, token)
                {
        ...
        {
            Response.Cookies.Add(new HttpCookie("EmailClientLogin_" + Authorization.UserId, "")
            {
        ...
        private static EmailLoginInfo GetLoginInfo()
        {
            HttpCookie cookie = System.Web.HttpContext.Current.Request.Cookies["EmailClientLogin_" + Authorization.UserId];
            if (cookie == null || string.IsNullOrEmpty(cookie.Value))
                throw new ValidationError("UnauthorizedAccess", "You need to login e-mail client first!");

            using (var ms = new MemoryStream(MachineKey.Unprotect(Convert.FromBase64String(cookie.Value), "EmailClientAuth")))
        ...
        public EmailUnreadCountResponse UnreadCount(EmailUnreadCountRequest request)
        {
            ...
            response.UnreadCount = LocalCache.Get<object>("LeftNavigationModel:UnreadMailCount:" +
                (Authorization.UserId.TryParseID() ?? -1L) + ":", TimeSpan.FromMinutes(1), () =>
                {
                    try
                    {
                        int unread = 0;
                        EmailController.UsingClient(client =>
                        {
        ...
        public EmailSuggestResponse Suggest(EmailSuggestRequest request)
        {
            ...
            var addressList = TwoLevelCache.Get("EmailSuggestList:" + Authorization.UserId, TimeSpan.FromHours(1), TimeSpan.FromHours(1), "Email",
                () =>
                {
        ...
        public ServiceResponse Compose(EmailComposeRequest request)
        {
            ...
            UsingClient(client =>
            {
                try
                {
                    client.GetFolder("Sent").Append(message, MessageFlags.Seen);
                }
                catch (Exception ex)
                {
                    ex.Log();
