...
namespace DAP.Membership.Pages
{
    ...
    public partial class AccountController : Controller
    {
                 ...
                 (user.TwoFactorAuth == Administration.TwoFactorAuthType.Email)))
            {
                ...
                if (user.TwoFactorAuth == Administration.TwoFactorAuthType.SMS)
                {
                    ...
                    Dependency.Resolve<Administration.ISMSService>().Send(
                        phoneNumber: mobile,
                        text: "Please use code " + data.TwoFactorCode + " for DAP login.",
                        reason: "Sent by DAP system for two factor authenication by SMS (" + user.Username + ")");
                ...
                else
                {
                    EmailHelper.Send(
                        address: user.Email,
                        subject: "Your two-factor authentication code for DAP login",
                        body: "Please use code " + data.TwoFactorCode + " for DAP login.");
        ...
        public ActionResult ImpersonateAs(string token)
        {
            ...
            using (var br = new BinaryReader(ms))
            {
                ...
                var currentClientId = Request.UserAgent + "|" + Request.UserHostAddress;
