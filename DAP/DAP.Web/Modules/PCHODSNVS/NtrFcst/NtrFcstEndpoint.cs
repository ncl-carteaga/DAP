...
namespace DAP.PCHODSNVS.Endpoints
{
    ...
    using MyRepository = Repositories.NtrFcstRepository;
    using MyRow = Entities.NtrFcstRow;
    ...
    [RoutePrefix("Services/PCHODSNVS/NtrFcst"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class NtrFcstController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.NtrFcstColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "NTRFcst_" +
            ...
            * the column location and match it to the system header using a Dictionary<string, int>. */
            ...
            for (var row = 2; row <= wsEnd.Row; row++)
            {
                try
                {
                    ...
                    var currentRow = uow.Connection.TryFirst<NtrFcstRow>(q => q
                        .Select(myFields.FcstTk)
                        .Where(myFields.CompanyCd == company_cd.ToString() && myFields.AccountPeriodNum == account_period_num.ToString()));

                    if (currentRow == null)
                        currentRow = new NtrFcstRow
                        {
                    ...
                    fieldTitle = myFields.NtrFcst.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.NtrFcst = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    entType = jImpHelp.entryType.Double; //designate the type of item
                    fieldTitle = myFields.PdsFcstTotal.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.PdsFcstTotal = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    if (currentRow.FcstTk == null)
                    {
                        new NtrFcstRepository().Create(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow
                        });
                    ...
                    {
                        new NtrFcstRepository().Update(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow,
                            EntityId = currentRow.FcstTk.Value
                        });
