...
namespace DAP.PCHODSNVS.Endpoints
{
    ...
    using MyRepository = Repositories.NtrBudgetByChannelRepository;
    using MyRow = Entities.NtrBudgetByChannelRow;
    ...
    [RoutePrefix("Services/PCHODSNVS/NtrBudgetByChannel"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class NtrBudgetByChannelController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.NtrBudgetByChannelColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "NTRBudgetByChannel_" +
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            if (!importedHeaders.Contains("accounting year", StringComparer.OrdinalIgnoreCase) == true)
            {
                response.ErrorList.Add("Missing Required Column accounting_year");
            }
            ...
            * the column location and match it to the system header using a Dictionary<string, int>. */
            ...
            for (var row = 2; row <= wsEnd.Row; row++)
            {
                try
                {
                    ...
                    int accounting_year = 0; // = myFields.AccountingYear;
                    string channel_desc = ""; // myFields.ChannelDesc
                    string channel_summ_desc = ""; // myFields.ChannelSummDesc
                    ...
                    fieldTitle = myFields.AccountingYear.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            accounting_year = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    entType = jImpHelp.entryType.String; //designate the type of item
                    fieldTitle = myFields.ChannelDesc.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            channel_desc = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    entType = jImpHelp.entryType.String; //designate the type of item
                    fieldTitle = myFields.ChannelSummDesc.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            channel_summ_desc = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }

                    var currentRow = uow.Connection.TryFirst<NtrBudgetByChannelRow>(q => q
                        .Select(myFields.BudgetbychannelTk)
                        .Where(myFields.CompanyCd == company_cd.ToString() && myFields.AccountingYear == accounting_year  && myFields.ChannelDesc == channel_desc.ToString() && myFields.ChannelSummDesc == channel_summ_desc.ToString()));

                    if (currentRow == null)
                        currentRow = new NtrBudgetByChannelRow
                        {
                            CompanyCd = company_cd.ToString(),
                            AccountingYear = accounting_year,
                            ChannelDesc = channel_desc,
                            ChannelSummDesc = channel_summ_desc
                        };
                    else
                    {
                    ...
                    entType = jImpHelp.entryType.Decimal; //designate the type of item
                    fieldTitle = myFields.NpdBudget.Title; //designate the field to be looked at
                    obj = myImpHelp.myExcelVal(row, myImpHelpExt.GetEntry(headerMap, fieldTitle).Value, worksheet);
                    if (obj != null)
                    {
                        importedValues.Add(obj);
                        ...
                        if (a != null)
                        {
                            currentRow.NpdBudget = a; //designate the field to be updated in the system
                        }
                        sysHeader.Clear();
                        importedValues.Clear();
                    }
                    ...
                    if (currentRow.BudgetbychannelTk == null)
                    {
                        new NtrBudgetByChannelRepository().Create(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow
                        });
                    ...
                    {
                        new NtrBudgetByChannelRepository().Update(uow, new SaveWithLocalizationRequest<MyRow>
                        {
                            Entity = currentRow,
                            EntityId = currentRow.BudgetbychannelTk.Value
                        });
